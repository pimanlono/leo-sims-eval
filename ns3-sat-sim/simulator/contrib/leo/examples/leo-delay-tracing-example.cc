...
int main (int argc, char *argv[])
{
  ...
  //SPE modification: to import the GWs from the same file as Hypatia
  std::string groundFile;

  cmd.AddValue("groundFile", "CSV file with ground station location", groundFile);
  ...
  //SPE modification: to implement +grid
  cmd.AddValue("nOrbits", "ns3::IslMockChannel::nOrbits");
  cmd.AddValue("nSatsPerOrbit", "ns3::IslMockChannel::nSatsPerOrbit");
  cmd.Parse (argc, argv);
...
//  LogComponentEnable("UdpServer",LOG_LEVEL_INFO);
  ...
  //SPE-modification: to import GW locations from file, if required 
  LeoGndNodeHelper ground;
  NodeContainer stations;
  if (!groundFile.empty())
    {
	stations=ground.Install(groundFile);
    }
  else
    {
	stations=ground.Install(latGws, lonGws);
    }
...
*/
  ...
  //SPE modification: ArpCache is implemented (as in Hypatia)

  //ipv4.Assign (utNet);
  Ipv4InterfaceContainer utIp=ipv4.Assign(utNet);

  NetDeviceContainer islNet;
  Ipv4InterfaceContainer islIp;
